...
## Notes
OffsetArrays.jl is a package that allows Fortan-like arrays.  This includes zero-based arrays, which can make modular arithmetic on array indicies more direct and less error prone.

In Julia % is the remainder operator, not the modulus as in other languages, like Python.  The remainder operator is equivalent to modulus for positive values but the result will differ by the modulus for negative values. Julia has the Base function mod(n, m) for modulus.
...
Note that, when trying to submit an exercise, make sure the solution is in the `$EXERCISM_WORKSPACE/julia/simple-cipher` directory.
